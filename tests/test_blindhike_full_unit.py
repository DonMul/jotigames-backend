...
class _RepositoryStub:
    def __getattr__(self, name: str):
        if name == "get_configuration":
            return lambda *_args, **_kwargs: {"finish_radius_meters": 25, "max_markers": 50}
        if name == "fetch_teams_by_game_id":
            return lambda *_args, **_kwargs: [{"id": "team-1", "name": "Team 1", "logo_path": "", "geo_score": 1}]
        if name == "fetch_markers_by_game_id":
            return lambda *_args, **_kwargs: []
        if name == "fetch_markers_by_game_and_team":
            return lambda *_args, **_kwargs: []
        if name == "get_team_location":
            return lambda *_args, **_kwargs: {"lat": 52.1, "lon": 5.1, "updated_at": datetime.now(UTC).isoformat()}
        if name in {"get_game_settings", "get_game_by_id"}:
            return lambda *_args, **_kwargs: {}
        if name in {"commit_changes", "rollback_on_error", "insert_marker_without_commit", "update_game_settings_without_commit", "increment_team_geo_score_without_commit"}:
...
def _default_arg(parameter: inspect.Parameter) -> Any:
    ...
    if name.endswith("_team_id"):
        return "team-2"
    if name in {"marker_id", "object_id", "claim_id"}:
        return "obj-1"
    if name in {"latitude", "lat", "target_lat", "marker_lat"}:
        return 52.1
    if name in {"longitude", "lon", "target_lon", "marker_lon"}:
        return 5.1
    if name in {"points", "max_markers", "radius_meters", "min_interval_seconds"}:
        return 1
    if name in {"metadata", "settings", "game_state", "config", "marker", "row", "values"}:
        return {}
    if name in {"rows", "marker_rows", "actions", "teams"}:
        return []
    if name == "finished_team_ids":
        return set()
    return "value"


def _bind_service_stubs(service: BlindHikeService) -> None:
...
def test_blindhike_service_all_class_functions_unit_invocable():
    service = BlindHikeService()
    _bind_service_stubs(service)

    executed: list[str] = []
    for name, method in BlindHikeService.__dict__.items():
    ...
    expected = [name for name, method in BlindHikeService.__dict__.items() if inspect.isfunction(method) and not name.startswith("__")]
    assert set(expected).issubset(set(executed))


def test_blindhike_repository_all_class_functions_unit_invocable():
    repo = BlindHikeRepository()
    table = _make_table()

    repo.get_game_by_id = lambda *_args, **_kwargs: {"id": "game-1"}
    repo.fetch_teams_by_game_id = lambda *_args, **_kwargs: [{"id": "team-1", "name": "Team 1"}]
    repo.get_team_by_game_and_id = lambda *_args, **_kwargs: {"id": "team-1", "latitude": 52.1, "longitude": 5.1}
    ...
    executed: list[str] = []
    for name, method in BlindHikeRepository.__dict__.items():
        if not inspect.isfunction(method) or name.startswith("__"):
            continue
    ...
    expected = [name for name, method in BlindHikeRepository.__dict__.items() if inspect.isfunction(method) and not name.startswith("__")]
    assert set(expected).issubset(set(executed))


def test_blindhike_module_build_router_registers_routes():
    module = BlindHikeModule(ws_publisher=_PublisherSpy())
