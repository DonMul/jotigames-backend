...
class _RepositoryStub:
    def __getattr__(self, name: str):
        ...
        if name in {"fetch_deliveries_by_game_id", "fetch_claims_by_game_id", "fetch_claims_by_game_and_team"}:
...
def _default_arg(parameter: inspect.Parameter) -> Any:
    ...
    if name.endswith("_team_id"):
        return "team-2"
    if name in {"delivery_id", "package_id", "object_id", "claim_id"}:
        return "obj-1"
    if name in {"points", "limit", "offset"}:
        return 1
    if name in {"metadata", "settings", "game_state", "config", "row", "values"}:
        return {}
    if name in {"rows", "actions", "teams", "deliveries"}:
        return []
    return "value"


def _bind_service_stubs(service: CourierRushService) -> None:
...
def test_courier_rush_service_all_class_functions_unit_invocable():
    service = CourierRushService()
    _bind_service_stubs(service)

    executed: list[str] = []
    for name, method in CourierRushService.__dict__.items():
    ...
    expected = [name for name, method in CourierRushService.__dict__.items() if inspect.isfunction(method) and not name.startswith("__")]
    assert set(expected).issubset(set(executed))


def test_courier_rush_repository_all_class_functions_unit_invocable():
    repo = CourierRushRepository()
    table = _make_table()
    ...
    executed: list[str] = []
    for name, method in CourierRushRepository.__dict__.items():
        if not inspect.isfunction(method) or name.startswith("__"):
            continue
    ...
    expected = [name for name, method in CourierRushRepository.__dict__.items() if inspect.isfunction(method) and not name.startswith("__")]
    assert set(expected).issubset(set(executed))


def test_courier_rush_get_team_bootstrap_includes_pickups_dropoffs_highscore():
    service = CourierRushService()
    _bind_service_stubs(service)
    pickup_row = {
        "id": "p-1", "title": "Pickup A", "latitude": 51.0, "longitude": 3.7,
        "radius_meters": 25, "points": 0, "marker_color": "#3b82f6", "is_active": True,
    }
    dropoff_row = {
        "id": "d-1", "title": "Dropoff A", "latitude": 52.0, "longitude": 3.8,
        "radius_meters": 30, "points": 5, "marker_color": "#16a34a", "is_active": True,
    }
    team_row = {"id": "team-1", "name": "Team 1", "logo_path": "", "geo_score": 7}
    service._repository.fetch_pickups_by_game_id = lambda *_a, **_k: [pickup_row]
    service._repository.fetch_dropoffs_by_game_id = lambda *_a, **_k: [dropoff_row]
    ...
    assert result["pickups"][0]["id"] == "p-1"
    assert result["pickups"][0]["marker_color"] == "#3b82f6"
    assert "dropoffs" in result and len(result["dropoffs"]) == 1
    assert result["dropoffs"][0]["id"] == "d-1"
    assert "highscore" in result
    assert result["highscore"][0]["score"] == 7


def test_courier_rush_get_team_bootstrap_empty():
    service = CourierRushService()
    _bind_service_stubs(service)
    service._repository.fetch_pickups_by_game_id = lambda *_a, **_k: []
    service._repository.fetch_dropoffs_by_game_id = lambda *_a, **_k: []
...
def test_courier_rush_module_build_router_registers_routes():
    module = CourierRushModule(ws_publisher=_PublisherSpy())
