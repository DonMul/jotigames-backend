...
class _RepositoryStub:
    def __getattr__(self, name: str):
        ...
        if name in {"fetch_nodes_by_game_id", "fetch_claims_by_game_id", "fetch_claims_by_game_and_team"}:
...
def _default_arg(parameter: inspect.Parameter) -> Any:
    ...
    if name.endswith("_team_id"):
        return "team-2"
    if name in {"node_id", "resource_id", "object_id", "claim_id", "code"}:
        return "obj-1"
    if name in {"points", "limit", "offset"}:
        return 1
    if name in {"metadata", "settings", "game_state", "config", "row", "values"}:
        return {}
    if name in {"rows", "actions", "teams", "nodes"}:
        return []
    return "value"


def _bind_service_stubs(service: ResourceRunService) -> None:
...
def test_resource_run_service_all_class_functions_unit_invocable():
    service = ResourceRunService()
    _bind_service_stubs(service)

    executed: list[str] = []
    for name, method in ResourceRunService.__dict__.items():
    ...
    expected = [name for name, method in ResourceRunService.__dict__.items() if inspect.isfunction(method) and not name.startswith("__")]
    assert set(expected).issubset(set(executed))


def test_resource_run_repository_all_class_functions_unit_invocable():
    repo = ResourceRunRepository()
    table = _make_table()
    ...
    executed: list[str] = []
    for name, method in ResourceRunRepository.__dict__.items():
        if not inspect.isfunction(method) or name.startswith("__"):
            continue
    ...
    expected = [name for name, method in ResourceRunRepository.__dict__.items() if inspect.isfunction(method) and not name.startswith("__")]
    assert set(expected).issubset(set(executed))


def test_resource_run_get_team_bootstrap_includes_nodes_and_highscore():
    service = ResourceRunService()
    _bind_service_stubs(service)
    node_row = {
        "id": "n-1", "title": "Gold", "latitude": 51.0, "longitude": 3.7,
        "radius_meters": 25, "points": 4, "marker_color": "#f59e0b", "is_active": True,
    }
    team_row = {"id": "team-1", "name": "Team 1", "logo_path": "", "geo_score": 8}
    service._repository.fetch_nodes_by_game_id = lambda *_a, **_k: [node_row]
    ...
    assert result["nodes"][0]["points"] == 4
    assert "highscore" in result
    assert result["highscore"][0]["score"] == 8


def test_resource_run_get_team_bootstrap_empty():
    service = ResourceRunService()
    _bind_service_stubs(service)
    service._repository.fetch_nodes_by_game_id = lambda *_a, **_k: []
...
def test_resource_run_module_build_router_registers_routes():
    module = ResourceRunModule(ws_publisher=_PublisherSpy())
