...
class GeoHunterModule(ApiModule, SharedModuleBase):
    ...
    def build_router(self) -> APIRouter:
        ...
        def update_location(
            ...
            """Update team location and return currently nearby POIs."""
            self._require_game(db, game_id)
            self._require_team_self_or_manage_access(db, game_id, team_id, principal)
            ...
            if should_publish:
                self._ws_publisher.publish(
                    "admin.geohunter.team.location.updated",
                    {
                        "game_id": game_id,
                        "team_id": team_id,
            ...
            return TeamLocationUpdateResponse(
                success=True,
                message_key=self._localize_message_key("geohunter.location.updated", locale),
                location=location,
                nearby_pois=nearby_pois,
                nearby_poi_ids=nearby_poi_ids,
            )
